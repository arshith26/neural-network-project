...
# so that `from matcher` and `from deep_matcher` resolve
...
# 2) Configuration
# ────────────────────────────────────────────────────────────────────────────────
JD_DIR     = "skillmap/jd"       # your JD folder
RES_DIR    = "skillmap/resumes"  # your resumes folder
PAIRS_FILE = "skillmap/pairs.csv" 
THRESHOLD  = 0.5                 # cutoff for “match” vs “no match”

# ────────────────────────────────────────────────────────────────────────────────
# 3) Read pairs.csv & detect label column
# ────────────────────────────────────────────────────────────────────────────────
df = pd.read_csv(PAIRS_FILE, dtype=str)
print("Original columns:", df.columns.tolist())

# heuristically find which column is ground‐truth 0/1
label_col = None
for c in df.columns:
    vals = set(df[c].dropna().unique())
    if c.lower().startswith("label") or vals <= {"0","1"}:
        label_col = c
        break

if label_col is None:
    raise RuntimeError("❌ Unable to detect label column in pairs.csv")
...
print(df[["jd_file","resume_file","label"]].head(), "\n")
...
for idx, row in df.iterrows():
    ...
    cos_sim   = calculate_similarity(res_text, jd_text)
